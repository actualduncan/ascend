...
// using this causes a crash... commenting out for now
/*
...
{
public:
    ComPtr<ID3D12Device14> spDevice;
    ComPtr<ID3D12GraphicsCommandList10> spCL;
    ComPtr<ID3D12CommandQueue> spCQ;
    ComPtr<ID3D12CommandAllocator> spCA;
    ...
    _Out_ ID3DBlob** ppCode)
{
    ...
    hr = s_pDxcCreateInstanceProc(CLSID_DxcCompiler, __uuidof(IDxcCompiler), reinterpret_cast<LPVOID*>(compiler.Get()));
    if (FAILED(hr))
    {
        PRINT("Failed to instantiate compiler.");
        return hr;
...
void InitDeviceAndContext(D3DContext& D3D)
{
    ...
    if (g_useWarpDevice)
    {
        ...
        VERIFY_SUCCEEDED(D3D12CreateDevice(warpAdapter.Get(), FL, IID_PPV_ARGS(&spDevice)));
    }
    else
    {
...
void FlushAndFinish(D3DContext& D3D)
{
    ...
    ID3D12CommandList* commandLists[] = { D3D.spCL.Get() };
    D3D.spCQ->ExecuteCommandLists(1, commandLists);

    VERIFY_SUCCEEDED(D3D.spCQ->Signal(D3D.spFence.Get(), ++D3D.FenceValue));
    ...
    VERIFY_SUCCEEDED(D3D.spCL->Reset(D3D.spCA.Get(), nullptr));
}
    ...
    bool doFlush)
{
    ...
    ComPtr<ID3D12Resource> pStagingResource;
    ...
    if (Size != UpdateSubresources(D3D.spCL.Get(), pResource, *ppStagingResource, 0, 0, 1, &SubResourceData))
    {
...
{
public:
    void Init(D3DContext& D3D, ComPtr<ID3D12StateObjectProperties1> spSO, LPCWSTR pWorkGraphName)
    {
        ...
        spWGProps = reinterpret_cast<ID3D12WorkGraphProperties*>(spSO.Get());
...
int main()
{
    ...
    try
    {
        ...
        ComPtr<ID3DBlob> library;
        VERIFY_SUCCEEDED(CompileDxilLibraryFromFile(g_File, L"lib_6_8", nullptr, 0, library.GetAddressOf()));
        ...
        ComPtr<ID3D12Resource> spGPUBuffer;
        ...
        WorkGraphContext WG;
        {
            ...
            CD3DX12_SHADER_BYTECODE libCode(library.Get());
        ...
        // Setup program
        D3D.spCL->SetComputeRootSignature(spRS.Get());
        ...
        // Readback GPU buffer
        Transition(D3D.spCL.Get(), spGPUBuffer.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
        D3D.spCL->CopyResource(spReadbackBuffer.Get(), spGPUBuffer.Get());
        FlushAndFinish(D3D);
